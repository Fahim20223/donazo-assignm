const ProjectAnalytics = () => {
  const chartData = [
    { day: 'S', height: 'h-24', color: 'bg-slate-100 dark:bg-slate-800', striped: true },
    { day: 'M', height: 'h-32', color: 'bg-primary', striped: false },
    { day: 'T', height: 'h-28', color: 'bg-primary/20', innerHeight: 'h-20', innerColor: 'bg-secondary', label: '74%' },
    { day: 'W', height: 'h-40', color: 'bg-primary', striped: false },
    { day: 'T', height: 'h-32', color: 'bg-slate-100 dark:bg-slate-800', striped: true },
    { day: 'F', height: 'h-36', color: 'bg-slate-100 dark:bg-slate-800', striped: true },
    { day: 'S', height: 'h-28', color: 'bg-slate-100 dark:bg-slate-800', striped: true },
  ];
      ...
      <div className="flex items-end justify-between h-48 gap-3">
        {chartData.map((bar, index) => (
          <div key={index} className="flex flex-col items-center gap-2 flex-1">
            <div className={`w-full ${bar.color} rounded-full ${bar.height} relative overflow-hidden`}>
              {bar.striped && (
                <div className="absolute bottom-0 w-full chart-striped h-full border-t-2 border-slate-300 dark:border-slate-600"></div>
              )}
              {bar.innerHeight && (
                <>
                  <div className={`absolute bottom-0 w-full ${bar.innerColor} ${bar.innerHeight} rounded-b-full`}></div>
